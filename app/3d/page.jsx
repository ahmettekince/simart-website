...
//     pts.push([pStartX, 0.25, pStartZ]); // Turu tamamla

//     // --- 2. ADIM: İÇ ZİGZAG (Fill) ---
//     // Tur bittiği yerden zigzag başlasın (Mesafe sw kadar içeri daralsın)
//     const ix0 = x0, ix1 = x1;
...
//         const shouldCont = (curr) => stepX > 0 ? curr <= targetX + 0.05 : curr >= targetX - 0.05;

//         for (let x = pStartX; shouldCont(x);) {
//             pts.push([x, 0.25, curZStart]);
//             pts.push([x, 0.25, curZEnd]);
//             [curZStart, curZEnd] = [curZEnd, curZStart];
//             if (Math.abs(x - targetX) < 0.1) break;
//             x = stepX > 0 ? Math.min(x + sw, targetX) : Math.max(x - sw, targetX);
//         }
//     }
...
//     const L = 0.8; // Kapı giriş/çıkış derinliği
//     if (dir === 'z') {
//         const pTarget = toPositive ? z + L : z - L;
//         return [[x, 0.25, z], [x, 0.25, pTarget]];
//     } else {
//         const pTarget = toPositive ? x + L : x - L;
...
//     if (type === "1+1") {
//         const suite = [];
//         suite.push({ name: "Salon", bounds: [[0.5, 4.5], [-4.5, 4.5]], doorPos: [0, 0], doorDir: 'x', isCenter: true });
//         suite.push({ name: "Mutfak", bounds: [[-4.5, -0.5], [-4.5, -0.5]], doorPos: [0, -2.5], doorDir: 'x', toPos: false, corridorX: 1.5 });
//         suite.push({ name: "Oda", bounds: [[-4.5, -0.5], [0.5, 4.5]], doorPos: [0, 2.5], doorDir: 'x', toPos: false, corridorX: 1.5 });
...
//     } else if (type === "3+2") {
//         const suite = [];
//         suite.push({ name: "Hol", bounds: [[-4, 1], [-6, 6]], doorPos: [0, 0], doorDir: 'x', isCenter: true });
//         suite.push({ name: "Ebeveyn", bounds: [[-9, -4], [2.25, 6]], doorPos: [-4, 3.12], doorDir: 'x', toPos: false });
//         suite.push({ name: "Misafir", bounds: [[-9, -4], [-1, 2.25]], doorPos: [-4, -0.12], doorDir: 'x', toPos: false });
//         suite.push({ name: "Çocuk", bounds: [[-9, -4], [-6, -1]], doorPos: [-4, -3.75], doorDir: 'x', toPos: false });
//         suite.push({ name: "Süit Giriş Holü", bounds: [[1, 4], [0, 2.25]], doorPos: [1, 1.1], doorDir: 'x', toPos: true });
//         suite.push({ name: "Mutfak", bounds: [[4, 9], [0, 2.25]], doorPos: [4, 1.1], doorDir: 'z', toPos: false, corridorX: 4 });
